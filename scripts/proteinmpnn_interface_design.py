...
parser.add_argument(
    ...
    type=str,
    default="",
    help="The name of a quiver file to run this metric on.",
)
...
default_ckpt = os.path.join(
    os.path.dirname(__file__), "/home/weights/ProteinMPNN_v48_noise_0.2.pt"
)
...
for pdb in struct_manager.iterate():
    if args.debug:
        ...
        try:
            ...
            seconds = int(time.time() - t0)
            print(
                f"Struct with tag {pdb} failed in {seconds} seconds with error: {type(e).__name__}"
            )
