    ...
    for i_des in range(design_startnum, design_startnum + sampler.inf_conf.num_designs):
        ...
        if sampler.inf_conf.quiver is not None:
            if out_prefix in tags:
                log.info(
                    f"Skipping this design because tag {out_prefix} already exists."
                )
                continue
        ...
        while True:
            ...
            for t in range(
                ...
                print(
                    ...
                    "".join(
                        conversion[i]
                        for i in torch.argmax(seq_t, dim=1)[sampler.ab_item.hotspots]
                    ),
                )
                if conf.antibody.terminate_bad_targeting is not None:
                    ...
                    mindist = torch.min(
                        dist, dim=1
                    ).values  # The min distance for each hotspot

                    overallmin = torch.min(
                        mindist
                    )  # The distance of the closest hotspot to a loop

                    print(
                        f"Overall min distance hotspot to designed loop: {overallmin}"
                    )
                    if (
                        conf.antibody.terminate_bad_targeting == t
                        and overallmin > conf.antibody.hotspot_termination_threshold
                    ):
                        print("Not targeting correctly")
                        failed += 1
                        if (
                            failed
                            >= conf.antibody.hotspot_termination_failures_permitted
                        ):
                            sys.exit(
                                "This set of inputs is not efficiently targeting the hotspots"
                            )
                        continue
        ...
        if (
            sampler.ab_design()
            and torch.any(sampler.ab_item.target_mask)
            and torch.any(sampler.ab_item.hotspots)
        ):
            # Loop through the hotspots, find the closest loop residue by Cb distance
            # And then average the distance over each of the hotspots. Report min and mean distance
            Cb = generate_Cbeta(
        ...
        if sampler.ab_design():
            ...
            if sampler.inf_conf.quiver is None:
                ...
                if torch.any(sampler.ab_item.target_mask) and torch.any(
                    sampler.ab_item.hotspots
                ):
        ...
        if sampler.inf_conf.write_trajectory:
            if sampler.inf_conf.quiver is not None:
                ...
                xtpdblines = writepdb_multi(
                    ...
                    return_pdblines=True,
                )
                px0pdblines = writepdb_multi(
                    None,
                    px0_xyz_stack,
                    bfacts,
                    final_seq.squeeze(),
