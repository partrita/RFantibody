    ...
    if os.path.isfile(filename):
        ...
        for p in os.getenv("PATH").split(os.pathsep):
            ...
            if os.path.isfile(filename):
                return filename
    print(
        "ERROR! Cannot locate %s at %s or at %s"
        % (exe, os.path.dirname(os.path.abspath(__file__)), os.getenv("PATH"))
    )
...
def usalign(mobile, target, args="", exe="", transform=1):
    ...
    try:
        process = subprocess.Popen(
            args, stdout=subprocess.PIPE, shell=True, universal_newlines=True
        )
        lines = process.stdout.readlines()
    except OSError:
        print(
            'Cannot execute "%s", please provide full path to USalign executable'
            % (args)
        )
        raise CmdException
    finally:
