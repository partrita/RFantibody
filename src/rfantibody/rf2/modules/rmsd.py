...
def tmalign_to_subset(pose1: "Pose", pose2: "Pose", subset: str) -> None:
    ...
    retlines = (
        subprocess.run([usalign, output1, output2, "-o", tag], stdout=subprocess.PIPE)
        .stdout.decode("utf-8")
        .split("\n")
    )

    score = max(
        float(l.split(" ")[1].strip()) for l in retlines if l.startswith("TM-score=")
    )
    # if score < 0.8:
...
def align_pose_to_subset(
    pose_complex: "Pose", aligned_subset: torch.Tensor, mask: torch.Tensor
) -> None:
    """
    ...
    if not mask.sum() == len(aligned_subset):
        raise ValueError(
            "Mask for the pose does not correctly index to the aligned xyz coordinates"
        )
...
def calc_prealigned_rmsd(
    pose1: "Pose", pose2: "Pose", mask: torch.Tensor[bool]
) -> float:
    """
