...
class Pose:
    ...
    def __post_init__(self):
        ...
        for attr_name, tensor in attributes_to_check.items():
            if tensor.shape[0] != self.length:
                raise ValueError(
                    f"{attr_name} length ({tensor.shape[0]}) does not match pose length ({self.length})"
                )
        if self.n_atoms not in [3, 14, 27]:  # without/with hydrogens
            raise ValueError(
                "xyz coordinates are wrong shape. Need either with hydrogens (27 atoms) or without (14 atoms), or without sidechains (3 atoms)"
            )
        if self.hotspots[self.antibody_mask].sum() != 0:
            raise ValueError(
                "Antibody chains do not have hotspots. These are defined on the target"
            )
    ...
    def pdb_idx(self) -> list:
        ...
        """
        chains = (
            ["H"] * sum(self.chain_dict["H"])
            + ["L"] * sum(self.chain_dict["L"])
            + ["T"] * sum(self.chain_dict["T"])
        )
...
class CDR:
    ...
    def __post_init__(self):
        ...
        for i in self.cdrs:
            if i.shape[0] != self.length:
                raise ValueError(
                    "Masks for all 6 CDRs must all be provided (even when they are all False) and must be the same length"
                )
            if type(i) is not torch.Tensor or i.dtype != torch.bool:
    ...
    def is_contiguous(mask: torch.Tensor) -> bool:
        ...
        transitions = torch.diff(
            mask, prepend=torch.tensor([False]), append=torch.tensor([False])
        )
...
def combine_chains(*, H=None, L=None, T=None) -> Pose:
    ...
    combined_attrs = {
        attr: torch.cat([getattr(i, attr) for i in chains], dim=0)
    ...
    chain_dict = OrderedDict(
        {
            ch: torch.cat(
                [getattr(i, "chain_dict")[ch] for i in chains if i is not None], dim=0
            )
            for ch in ["H", "L", "T"]
        }
    )
...
def concatenate_cdr_tensors(H=None, L=None, T=None) -> dict:
    ...
    chains = [H, L, T]
    concatenated = {
        cdr: torch.cat([getattr(chain, cdr) for chain in chains if chain is not None])
...
def pose_from_RF_output(output: dict, input_pose: Pose) -> Pose:
    ...
    """
    return Pose(
        xyz=output["xyz_prev"][0],
        seq=input_pose.seq,
        atom_mask=input_pose.atom_mask,
        cdrs=input_pose.cdrs,
        idx=input_pose.idx,
        chain_dict=input_pose.chain_dict,
    )
...
def masks_from_pdb_idx(pdb_idx: list) -> dict[str, torch.Tensor]:
    ...
    return OrderedDict(
        {
            ch: torch.Tensor([i[0] == ch for i in pdb_idx]).bool()
            for ch in ["H", "L", "T"]
        }
    )
    ...
    if conf.input.quiver is not None:
        ...
        if not util.check_if_remarked(conf.input.pdb):
            raise NotImplementedError(
                f"Only HLT remarked pdb files are currently supported. {conf.input.pdb} is not remarked"
            )
        yield (
            pose_from_remarked(conf.input.pdb),
            os.path.splitext(os.path.basename(conf.input.pdb))[0],
        )
