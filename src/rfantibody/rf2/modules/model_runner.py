...
class AbPredictor(Predictor):
    ...
    def __init__(self, conf: HydraConfig, preprocess_fn: Preprocess, device="cuda:0"):
        ...
        super().__init__(
            self.conf.model.model_weights,
            self.device,
            model_param=self.conf.model_param,
        )
        self.return_rmsds = any(
            [
                var is not None
                for var in [conf.input.pdb, conf.input.pdb_dir, conf.input.quiver]
            ]
        )
    ...
    def _process_output(
        self, output_i: dict, output_pose_i: Pose, input_pose: Pose
    ) -> OrderedDict:
        """
...
def get_rmsds(pose1: Pose, pose2: Pose, metrics: dict) -> None:
    ...
    metrics["target_aligned_antibody_rmsd"] = rmsd.calc_prealigned_rmsd(
        pose1, pose2, pose1.antibody_mask
    )
    metrics["target_aligned_cdr_rmsd"] = rmsd.calc_prealigned_rmsd(
        pose1, pose2, pose1.cdrs.mask_1d
    )

    # framework-aligned monomer rmsds
    rmsd.tmalign_to_subset(pose1, pose2, subset="framework")
    metrics["framework_aligned_antibody_rmsd"] = rmsd.calc_prealigned_rmsd(
        pose1, pose2, pose1.antibody_mask
    )
    metrics["framework_aligned_cdr_rmsd"] = rmsd.calc_prealigned_rmsd(
        pose1, pose2, pose1.cdrs.mask_1d
    )
    ...
    for loop in pose1.cdrs.cdr_names():
        metrics[f"framework_aligned_{loop}_rmsd"] = rmsd.calc_prealigned_rmsd(
            pose1, pose2, getattr(pose1.cdrs, f"{loop}")
        )
...
def write_output(to_write: OrderedDict, tag: str, conf: HydraConfig) -> None:
    ...
    if sum([var is not None for var in [conf.output.pdb_dir, conf.output.quiver]]) != 1:
        raise ValueError(
            "Exactly one of output.pdb_dir or output.quiver must be specified"
        )
    ...
    for key, val in to_write.items():
        ...
        if qv:
            ...
            pdblines = [
                f"QV_{line}" if line.startswith("SCORE") else line for line in pdblines
            ]
