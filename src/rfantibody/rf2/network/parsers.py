...
def read_templates(
    qlen, ffdb, hhr_fn, atab_fn, templ_to_use=[], offset=0, n_templ=10, random_noise=5.0
):
    xyz_t, mask, qmap, t1d, seq, ids = parse_templates_raw(
        ffdb, hhr_fn, atab_fn, templ_to_use, max_templ=max(n_templ, 20)
    )
    npick = min(n_templ, len(ids))
    if npick < 1:  # no templates
        xyz = (
            INIT_CRDS.reshape(1, 1, 27, 3).repeat(npick, qlen, 1, 1)
            + torch.rand(npick, qlen, 1, 3) * random_noise
        )
...
def read_template_pdb(qlen, templ_fn, align_conf=1.0):
    ...
    conf = torch.where(
        mask, torch.tensor(align_conf).float(), torch.zeros(qlen, dtype=xyz.dtype)
    )
