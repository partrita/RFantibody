...
def make_graph_w_2nodes(xyz, pair, idx, top_k_BB=64, top_k_SC=64, kmin=32, eps=1e-6):
    ...
    G.edata["rel_pos"] = (
        xyz.view(-1, 3)[tgt] - xyz.view(-1, 3)[src]
...
def make_topk_graph(xyz, pair, idx, top_k=128, kmin=32, eps=1e-6):
    ...
    # get top_k neighbors
    D_neigh, E_idx = torch.topk(
        D, min(eff_top_k, L - 1), largest=False
    )  # shape of E_idx: (B, L, top_k)
...
class XYZConverter(nn.Module):
    ...
    def get_torsions(self, xyz_in, seq, mask_in=None):
        ...
        # chis
        ti0 = torch.gather(
            xyz, 2, self.torsion_indices[seq, :, 0, None].repeat(1, 1, 1, 3)
        )
        ti1 = torch.gather(
            xyz, 2, self.torsion_indices[seq, :, 1, None].repeat(1, 1, 1, 3)
        )
        ti2 = torch.gather(
            xyz, 2, self.torsion_indices[seq, :, 2, None].repeat(1, 1, 1, 3)
        )
        ti3 = torch.gather(
            xyz, 2, self.torsion_indices[seq, :, 3, None].repeat(1, 1, 1, 3)
        )
