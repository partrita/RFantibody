...
def MSAFeaturize(
    msa, ins, params={"MAXLAT": 128, "MAXSEQ": 512}, p_mask=0.15, eps=1e-6, L_s=[]
):
    """
    ...
    if N - Nclust >= params["MAXSEQ"]:  # there are enough extra sequences
        ...
        N - Nclust < 1
    ):  # no extra sequences, use all masked seed sequence as extra one
        Nextra = Nclust
    ...
    agreement = torch.matmul(
        (count_extra[:, :, None] * msa_extra_onehot).view(N_extra_pool, -1),
