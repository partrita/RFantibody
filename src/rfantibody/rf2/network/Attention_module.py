    ...
    def forward(self, src, stride=0):
        ...
        if STRIDE > 0 and STRIDE < L:
            ...
            for i in range((L - 1) // STRIDE + 1):
                cols = torch.arange(
                    i * STRIDE, min((i + 1) * STRIDE, L), device=src.device
                )
...
class Attention(nn.Module):
    ...
    def forward(self, query, key, value, stride):
        ...
        if STRIDE > 0 and STRIDE < B:
            out = torch.zeros(
                (B, Q, self.d_out), device=query.device, dtype=query.dtype
            )
            for i in range((B - 1) // STRIDE + 1):
                batches = torch.arange(
                    i * STRIDE, min((i + 1) * STRIDE, B), device=query.device
                )
    ...
    def forward(self, msa, MSANorm, stride=0):
        ...
        if STRIDE > 0 and STRIDE < N:
            for i in range((N - 1) // STRIDE + 1):
                rows = torch.arange(
                    i * STRIDE, min((i + 1) * STRIDE, N), device=msa.device
                )
    ...
    def forward(self, msa, pair, stride_n, stride_l):
        ...
        if STRIDE_L < L or STRIDE_N < N:
            ...
            for i in range((N - 1) // STRIDE_N + 1):
                rows = torch.arange(
                    i * STRIDE_N, min((i + 1) * STRIDE_N, N), device=pair.device
                )
            ...
            for i in range((L - 1) // STRIDE_L + 1):
                rows = torch.arange(
                    i * STRIDE_L, min((i + 1) * STRIDE_L, L), device=pair.device
                )
            ...
            out = torch.zeros(
                (B, N, L, self.h * self.dim), device=pair.device, dtype=pair.dtype
            )
            for i in range((L - 1) // STRIDE_L + 1):
                slices = torch.arange(
                    i * STRIDE_L, min((i + 1) * STRIDE_L, L), device=pair.device
                )  # rows in value, cols in out
                msa_i = self.norm_msa(msa[:, :, slices])
                value_i = self.to_v(msa_i).reshape(B, N, -1, self.h, self.dim)
                out += einsum("bijh,bnjhd->bnihd", attn[:, :, slices], value_i).reshape(
                    B, N, L, -1
                )

            outg = torch.zeros(
                (B, N, L, self.dim_out), device=pair.device, dtype=pair.dtype
            )
            for i in range((L - 1) // STRIDE_L + 1):
                slices = torch.arange(
                    i * STRIDE_L, min((i + 1) * STRIDE_L, L), device=pair.device
...
class MSAColAttention(nn.Module):
    ...
    def forward(self, msa, stride):
        ...
        if STRIDE > 0 and STRIDE < L:
            out = torch.zeros(
                (B, N, L, self.dim_out), device=msa.device, dtype=msa.dtype
            )
            for i in range((L - 1) // STRIDE + 1):
                cols = torch.arange(
                ...
                out_i = einsum("bihqk,bkihd->bqihd", attn_i, value_i).reshape(
                    B, N, -1, self.dim_out
                )
                out_i = gate_i * out_i
...
class BiasedAxialAttention(nn.Module):
    ...
    def forward(self, pair, bias, stride):
        ...
        if STRIDE > 0 and STRIDE < L:
            ...
            out = torch.zeros(
                (B, L, L, self.dim_out), device=pair.device, dtype=pair.dtype
            )
            for i in range((L - 1) // STRIDE + 1):
                rows = torch.arange(
                    i * STRIDE, min((i + 1) * STRIDE, L), device=pair.device
                ...
                for j in range((L - 1) // STRIDE + 1):
                    ...
                    out_ij = einsum(
                        "bijh,bnjhd->bnihd", attn[:, cols], value_i
                    ).reshape(B, NR, NC, -1)
...
class TriangleMultiplication(nn.Module):
    ...
    def forward(self, pair, stride):
        ...
        if STRIDE > 0 and STRIDE < L:
            out = torch.zeros(
                (B, L, L, self.d_out), device=pair.device, dtype=pair.dtype
            )
            for i in range((L - 1) // STRIDE + 1):
                rows = torch.arange(
                    i * STRIDE, min((i + 1) * STRIDE, L), device=pair.device
                ...
                for j in range((L - 1) // STRIDE + 1):
                    ...
                    pair_ij = self.norm(pair[:, rows[:, None], cols[None, :]]).to(
                        dtype=pair.dtype
                    )
