...
class MSAPairStr2MSA(nn.Module):
    ...
    def forward(self, msa, pair, rbf_feat, state, strides):
        ...
        if self.training:
            ...
            msa = msa + self.drop_row(
                self.row_attn(msa, pair, stride_msarow_n, stride_msarow_l)
            )
...
class PairStr2Pair(nn.Module):
    ...
    def subblock(self, OP, pair, rbf_feat, crop):
        ...
        for i in range(ncrops):
            ...
            countnew.index_add_(
                1,
                iUL,
                torch.ones((N, iUL.shape[0]), device=countnew.device, dtype=torch.int),
            )

        pairnew = pairnew.reshape(N, L, L, -1) / countnew.reshape(N, L, L, -1)

        for i in range((L - 1) // crop + 1):
            rows = torch.arange(i * crop, min((i + 1) * crop, L), device=pair.device)[
                :, None
            ]
            for j in range((L - 1) // crop + 1):
                cols = torch.arange(
                    j * crop, min((j + 1) * crop, L), device=pair.device
                )[None, :]
    ...
    def forward(self, pair, rbf_feat, state, strides, crop=-1):
        ...
        if STRIDE > 0 and STRIDE < L:
            rbf_feat_out = torch.zeros(
                (B, L, L, self.d_pair), device=rbf_feat.device, dtype=rbf_feat.dtype
            )
            for i in range((L - 1) // STRIDE + 1):
                rows = torch.arange(
                    i * STRIDE, min((i + 1) * STRIDE, L), device=pair.device
                )
                for j in range((L - 1) // STRIDE + 1):
                    cols = torch.arange(
                        j * STRIDE, min((j + 1) * STRIDE, L), device=pair.device
                    )
                    NR, NC = rows.shape[0], cols.shape[0]
                    gate_ij = einsum(
                        "bli,bmj->blmij", left[:, rows], right[:, cols]
                    ).reshape(B, NR, NC, -1)
                    gate_ij = torch.sigmoid(self.to_gate(gate_ij))
                    rbf_feat_i = self.emb_rbf(rbf_feat[:, rows[:, None], cols[None, :]])
                    rbf_feat_out[:, rows[:, None], cols[None, :]] = (
                        rbf_feat_i * gate_ij
                    ).to(rbf_feat.dtype)
        ...
        if crop > 0 and crop <= L:
            ...
            if self.training:
                ...
                pair = pair + self.drop_row(
                    self.row_attn(pair, rbf_feat, stride_biasedax)
                )
                pair = pair + self.drop_col(
                    self.col_attn(pair, rbf_feat, stride_biasedax)
                )
        ...
        if STRIDE > 0 and STRIDE < L:
            for i in range((L - 1) // STRIDE + 1):
                ...
                for j in range((L - 1) // STRIDE + 1):
                    ...
                    out_ij = einsum("bsli,bsmj->blmij", left, right).reshape(
                        B, NR, NC, -1
                    )
...
def update_symm_Rs(Rs, Ts, Lasu, symmsub_in, symmsub, symmRs):
    def dist_error(R0, T0, Rs, Ts):
        ...
        Tcorr = (
            torch.einsum("ij,brj->bri", R0, Ts[:, :Lasu] - Tcom)
            + Tcom
            + 10.0 * T0[None, None, :]
        )
    ...
    Rs = torch.einsum(
        "sij,brjk,slk->bsril", symmRs[symmsub], Rs[:, :Lasu], symmRs[symmsub_in]
    )
...
class Str2Str(nn.Module):
    ...
    def forward(
        ...
        if STRIDE > 0 and STRIDE < L:
            node = torch.zeros(
                (B, L, self.n_node), device=msa.device, dtype=torch.float32
            )  # force f32
            for i in range((L - 1) // STRIDE + 1):
                rows = torch.arange(
                    i * STRIDE, min((i + 1) * STRIDE, L), device=msa.device
                )
                seq_i = self.norm_msa(msa[:, 0, rows])
                state[:, rows] = self.norm_state(state[:, rows]).to(
                    state.dtype
                )  # update inplace
        ...
        if STRIDE > 0 and STRIDE < L:
            edge = torch.zeros(
                (B, L, L, self.n_edge), device=msa.device, dtype=msa.dtype
            )
            for i in range((L - 1) // STRIDE + 1):
                rows = torch.arange(
                    i * STRIDE, min((i + 1) * STRIDE, L), device=msa.device
                ...
                for j in range((L - 1) // STRIDE + 1):
                    cols = torch.arange(
                        j * STRIDE, min((j + 1) * STRIDE, L), device=msa.device
                    )

                    NR, NC = rows.shape[0], cols.shape[0]
                    pair_ij = self.norm_pair(pair[:, rows[:, None], cols[None, :]])
                    rbf_feat_ij = rbf(
                        torch.cdist(xyz[:, rows, 1], xyz[:, cols, 1])
                    ).reshape(B, NR, NC, -1)
                    rbf_feat_ij = torch.cat(
                        (rbf_feat_ij, seqsep[:, rows[:, None], cols[None, :]]), dim=-1
                    )
                    edge_ij = self.embed_edge1(pair_ij) + self.embed_edge2(rbf_feat_ij)
                    edge_ij += self.ff_edge(edge_ij, stride_ff_s2s)
                    edge[:, rows[:, None], cols[None, :]] = self.norm_edge(edge_ij).to(
                        msa.dtype
                    )
        else:
        ...
        G, edge_feats = make_topk_graph(
            xyz[:, :, 1, :].detach(), edge, idx_in, top_k=top_k
        )
        edge = None
...
class IterBlock(nn.Module):
    ...
    def forward(
        ...
        rbf_feat = torch.zeros(
            (B, L, L, self.d_rbf), device=msa.device, dtype=msa.dtype
        )
        for i in range((L - 1) // STRIDE + 1):
            rows = torch.arange(i * STRIDE, min((i + 1) * STRIDE, L), device=msa.device)
            for j in range((L - 1) // STRIDE + 1):
                ...
                NR, NC = rows.shape[0], cols.shape[0]
                rbf_feat_ij = (
                    rbf(torch.cdist(xyz[:, rows, 1], xyz[:, cols, 1])).reshape(
                        B, NR, NC, -1
                    )
        ...
        if use_checkpoint:
            ...
            pair = checkpoint.checkpoint(
                create_custom_forward(self.msa2pair),
                msa,
                pair,
                strides,
                use_reentrant=True,
            ...
            pair = checkpoint.checkpoint(
                create_custom_forward(self.pair2pair),
                pair,
                rbf_feat,
                state,
                strides,
            ...
            if low_vram and not self.training:
                msa = (
                    msa.cpu()
                )  # temporarily move msa to CPU to free more memory for p2p
        ...
        if symmsub is not None and symmsub.shape[0] > 1:
            R, T, pair, symmsub = update_symm_subs(
                R, T, pair, symmids, symmsub_in, symmsub, symmRs, symmmeta
            )
...
class IterativeSimulator(nn.Module):
    ...
    def forward(
        ...
        for i_m in range(self.n_ref_block):
            ...
            if symmsub_in is not None and symmsub_in.shape[0] > 1:
                R_in, T_in = update_symm_Rs(
                    R_in, T_in, Lasu, symmsub_in, symmsub, symmRs
                )
