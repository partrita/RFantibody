...
def mask_ss(ss, min_mask=0, max_mask=0.75):
    ...
    while (
        ...
        try:
            ...
            counter += 1
        except IndexError:  # Replace with the specific exception you expect
            counter += 1
    ss = torch.tensor(ss)
    ...
    if ss is not None:
        ...
        if compatible is True:
            ...
            block_adj = construct_block_adj_matrix(
                torch.from_numpy(np.copy(ss)).float(), xyz, bb_mask
            )
...
def process_complex_ss(ssA, ssB, xyz, bb_mask):
    ...
    if (
        ssA is not None
        and ssB is not None
        and len(ssA["ss"]) + len(ssB["ss"]) == torch.sum(bb_mask)
    ):
        joint_dict = {}
        for key in ssA.keys():
            if key != "idx":
