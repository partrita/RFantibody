...
def center_and_realign_missing(xyz, mask_t):
    ...
    center_CA = (mask[..., None] * xyz[:, 1]).sum(dim=0) / (
        mask[..., None].sum(dim=0) + 1e-5
    )  # (3)
...
def get_mu_xt_x0(xt, px0, t, schedule, alphabar_schedule, eps=1e-6):
    ...
    a = (
        (torch.sqrt(alphabar_schedule[t_idx - 1] + eps) * schedule[t_idx])
        / (1 - alphabar_schedule[t_idx])
    ) * px0_ca
    b = (
        (torch.sqrt(1 - schedule[t_idx] + eps) * (1 - alphabar_schedule[t_idx - 1]))
        / (1 - alphabar_schedule[t_idx])
    ) * xt_ca
    mu = a + b
...
def get_cdr_hotspots(
    ...
    assert xyz.shape[0] == target_mask.shape[0], "Target mask is the wrong size for xyz"
    if loop_mask is not None:
        assert xyz.shape[0] == loop_mask.shape[0], "Loop mask is the wrong size for xyz"
        assert loop_mask.dtype == torch.bool, "Loop mask should be boolean"
        assert (target_mask * loop_mask).sum() == 0, "Target mask and loop mask overlab"
    assert target_mask.dtype == torch.bool, "Target mask should be boolean"
    ...
    if loop_mask is None or loop_mask.sum() == 0:  # edge case where no loops
        dist = torch.cdist(
            xyz[target_mask, 4], xyz[~target_mask, 4]
        )  # [target_L, loop_L]
    else:
    ...
    def convert(a):
        return torch.from_numpy(np.array(a)).bool()
    ...
    for i, s in enumerate(seq):
        ...
        for j, atm_j in enumerate(atms):
            if atm_j is not None and not np.isnan(xyz[i, j, 0]):
                f.write(
                    "%-6s%5s %4s %3s %s%5s   %8.3f%8.3f%8.3f%6.2f%6.2f\n"
                    % (
                        "ATOM",
                        ctr,
                        atm_j,
