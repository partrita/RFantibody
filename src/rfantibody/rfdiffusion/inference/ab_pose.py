...
class AbPose:
    ...
    def parse_hotspots(self, hotspot_res: str) -> torch.Tensor:
        ...
        if hotspot_res is None:
            ic(
                "WARNING! No hotspot residues were provided to the model at inference time"
            )
            return hotspot_idx

        assert all([i[0].isalpha() for i in hotspot_res]), (
    ...
    def target_from_HLT(self, pdb_path: str) -> None:
        ...
        # Determine residue positions of the loops
        for l in self.cdr_names:
            assert pose["loop_masks"][l].sum() == 0, (
    ...
    def from_chothia(self, pdb_path: str, chain_dict: dict) -> None:
        ...
        for chain in chain_dict:
            ...
            if type(chain_dict[chain]) is not tuple:
    ...
    def expand_loop(
        ...
        extra_xyz = (
            ...
            + torch.rand(length_difference, 1, 3) * random_noise
        )  # [padded_L,27,3]
    ...
    def update_features(self, features, loop_masks, loop_names, sampled_lengths):
        ...
        for n in loop_names:
            assert loop_masks[n].any(), (
                f"No residues found for loop {n}. Running with a subset of loops from one chain is not currently supported"
            )
        ...
        while idx < features["xyz"].shape[0]:
            if loop_idx < len(loop_names) and loop_masks[loop_names[loop_idx]][idx]:
                ...
                # If we have gotten here then the current loop is changing in length
                    ...
                    # Unreachable code
                    raise Exception(
                        "Something strange has occured in the loop length modification code"
                    )
