...
def slerp_update(r_t, r_0, t, mask=0):
    ...
    interp_rot = (interp_frame.as_matrix().squeeze() @ np.linalg.inv(r_t.squeeze()))[
        None, ...
    ]
...
def get_next_frames(
    ...
    if assert_vectorized_is_equal:
        ...
        for i in range(len(xt)):
            ...
            if so3_type == "igso3":
                r_t_next = diffuser.so3_diffuser.reverse_sample(
                    r_t, r_0, t, mask=mask_i, noise_level=noise_scale
                )[None, ...]
...
class DecodeSchedule:
    ...
    def get_decode_positions(self, t_idx, px0):
        ...
        L = px0.shape[0]
        assert t_idx < len(
            self.ndecode_per_step
        )  # taken care of outside this class in sampling loop
        ...
        if self.mode == "distance_based":
            ...
            for i in range(N):
                ...
                self.visible[decode_idx] = (
                    True  # set this now because get_next_idx depends on it
                )
                self.T[decode_idx] = (
                    t_idx + 1
                )  # now that we know this residue is decoded, set its big T value
...
class Denoise:
    ...
    def __init__(
        ...
        self.frame_noise_schedule_type = (
            ca_noise_schedule_type  # frame_noise_schedule_type
        )
        ...
        chi_abar_T = 1e-3
        assert (
            max_chi_T >= aa_decode_steps
        )  # assume never decoding for more than 100 steps, change if otherwise
        self.chi_beta_T = get_chi_betaT(
            max_chi_T, chi_b_0, chi_abar_T, method="cosine"
        )  # precalculate chi beta schedules for dynamic T
        ...
        if seq_diffuser is None:
            self.decode_scheduler = DecodeSchedule(
                L, visible, aa_decode_steps, mode="distance_based"
            )
    ...
    @staticmethod
    def get_dynamic_mu_sigma(
        chi_t, chi_0, T, t, chi_beta_T, beta_0, schedule_type="cosine"
    ):
        """
        ...
        for i, _ in enumerate(chi_t):
            ...
            if t <= cur_T:  # it's a valid position to find a beta for
                ...
                beta_schedule, alpha_schedule, abar_schedule = get_beta_schedule(
                    cur_T, beta_0, beta_T, schedule_type
                )
    ...
    def get_next_torsions(
        self, xt, px0, seq_t, seq_logit_px0, t, diffusion_mask=None, noise_scale=1.0
    ):
        """
        ...
        # Xt torsions
        torsions_sincos_xt, torsions_alt_sincos_xt, tors_mask_xt, tors_planar_xt = (
            get_torsions(
                xt_full[None], seq_t[None], TOR_INDICES, TOR_CAN_FLIP, REF_ANGLES
            )
        )
        torsions_angle_xt = torch.atan2(
            torsions_sincos_xt[..., 1], torsions_sincos_xt[..., 0]
        ).squeeze()
        torsions_angle_xt_alt = torch.atan2(
            torsions_alt_sincos_xt[..., 1], torsions_alt_sincos_xt[..., 0]
        ).squeeze()

        # pX0 torsions - uses same seq_t to calculate
        torsions_sincos_px0, torsions_alt_sincos_px0, tors_mask_px0, tors_planar_px0 = (
            get_torsions(
                px0_full[None], seq_t[None], TOR_INDICES, TOR_CAN_FLIP, REF_ANGLES
            )
        )
        torsions_angle_px0 = torch.atan2(
            torsions_sincos_px0[..., 1], torsions_sincos_px0[..., 0]
        ).squeeze()
        ...
        a = (
            th_min_angle(
                torsions_angle_xt * 180 / np.pi,
                torsions_angle_px0 * 180 / np.pi,
                radians=False,
            )
            * np.pi
            / 180
        ...
        b = (
            th_min_angle(
                torsions_angle_xt_alt * 180 / np.pi,
                torsions_angle_px0 * 180 / np.pi,
                radians=False,
            )
            * np.pi
        ...
        torsions_xt_mindiff = torch.where(
            condition, torsions_angle_xt, torsions_angle_xt_alt
        )
        ...
        if self.chi_type == "interp":
            ...
            T = self.decode_scheduler.idx2steps.repeat(
                10, 1
            ).T  # L,10 repeat of idx2steps
            T_flat = T.flatten()

            # sample mean angles and stddevs based on
            # start angle, end angle, t and dynamic T's

            mu, sigma = self.get_dynamic_mu_sigma(
                chi_t=start,
                chi_0=end,
                T=T_flat,
                t=t,
            ...
            next_torsions = torch.normal(mu, sigma * noise_scale).reshape(L, 10) % (
                2 * np.pi
            )
            ...
            if diffusion_mask is not None:
                ...
                next_torsions[diffusion_mask, ...] = torsions_xt_mindiff[
                    diffusion_mask, ...
                ]
        ...
        next_torsions_trig = torch.stack(
            [torch.cos(next_torsions), torch.sin(next_torsions)], dim=-1
        )
    ...
    def get_next_pose(
        ...
        assert (px0.shape[1] == 14) or (
            px0.shape[1] == 27
        )  # need full atom rep for torsion calculations
        ...
        grad_ca = self.get_potential_gradients(
            seq_t.clone(), xt.clone(), diffusion_mask=diffusion_mask
        )
        ...
        if diffuse_sidechains:
            if self.seq_diffuser:
                raise NotImplementedError(
                    "Sidechain diffusion and sequence diffusion cannot be performed at the same time"
                )

            seq_t = torch.argmax(seq_t, dim=-1).cpu()  # [L]
            pseq0 = torch.argmax(pseq0, dim=-1).cpu()  # [L]
            torsions_next, seq_next = self.get_next_torsions(
                xt,
                px0,
                seq_t,
                pseq0,
            ...
            _, fullatom_next = get_allatom(
                seq_t[None], frames_next[None], torsions_next[None]
            )
            seq_next = torch.nn.functional.one_hot(seq_next, num_classes=22).float()
            ...
            if self.seq_diffuser:
                seq_next = self.seq_diffuser.get_next_sequence(
                    seq_t[:, :20], pseq0, t, seq_diffusion_mask
                )  # [L,20]
...
def preprocess(seq, xyz_t, t, T, ppi_design, binderlen, target_res, device):
    ...
    t1d[:, :, :, :21] = nn.one_hot(torch.where(seq == 21, 20, seq), num_classes=21)[
        None, None
    ]
    ...
    conf = torch.where(seq == 21, 1 - t / T, 1.0)[None, None, ..., None]
...
def parse_pdb_lines(
    lines, parse_hetatom=False, ignore_het_h=True, parse_ab_loops=False
):
    ...
    for l in lines:
        if l[:21] == "REMARK PDBinfo-LABEL:":
            ...
            if loop_type not in loops:
                print(
                    f"WARNING: encountered unknown loop type: {loop_type} while parsing pdb. Skipping this loop"
                )
                continue
...
def get_idx0_hotspots(mappings, ppi_conf, binderlen):
    ...
    if binderlen > 0:
        if ppi_conf.hotspot_res is not None:
            assert all([i[0].isalpha() for i in ppi_conf.hotspot_res]), (
...
def recycle_schedule(T, rec_sched=None, num_designs=1):
    ...
    if rec_sched is not None:
        ...
        assert all(indices[i] > indices[i + 1] for i in range(len(indices) - 1)), (
            "Recycle schedule indices must be in decreasing order"
        )
        for part in parts:
            idx, num = part.split(":")
...
class BlockAdjacency:
    ...
    def __init__(self, conf, num_designs):
        ...
        if conf.scaffold_list is not None:
            if type(conf.scaffold_list) is list:
...
class Target:
    ...
    def contig_crop(self, contig_crop, residue_offset=200):
        ...
        # sanity check
        assert np.sum(self.pdb["hotspots"]) == np.sum(self.pdb["hotspots"][mask]), (
