...
from rfantibody.rfdiffusion.chemical import seq2chars
...
class Sampler:
    ...
    def initialize(self, conf: DictConfig):
        ...
        if self._conf.seq_diffuser.seqdiff is None:
            ...
            assert self._conf.preprocess.seq_self_cond is False, (
                "AR decoding does not make sense with sequence self cond"
            )
            ...
            sys.exit(
                f"Seq Diffuser of type: {self._conf.seq_diffuser.seqdiff} is not known"
            )
        ...
        recycle_schedule = (
            str(self.inf_conf.recycle_schedule)
            if self.inf_conf.recycle_schedule is not None
            else None
        )
    ...
    def assemble_config_from_chk(self) -> None:
        ...
        overrides.append(f"diffuser.T={self._conf.diffuser.T}") if not any(
            i.startswith("diffuser.T=") for i in overrides
        ) else None
        ...
        if "config_dict" in self.ckpt.keys():
            ...
            for cat in ["model", "diffuser", "seq_diffuser", "preprocess", "antibody"]:
                ...
                for key in self._conf[cat]:
                    if (
                        key == "chi_type"
                        and self.ckpt["config_dict"][cat][key] == "circular"
                    ):
                        ic(
                            "---------------------------------------------SKIPPPING CIRCULAR CHI TYPE"
                        )
                        continue
                    try:
                        print(
                        ...
                        print(
                            f"WARNING: config {cat}.{key} is not saved in the checkpoint. Check that conf.{cat}.{key} = {self._conf[cat][key]} is correct"
                        )
            # add back in overrides again
            for override in overrides:
                if override.split(".")[0] in [
                    "model",
                    "diffuser",
                    "seq_diffuser",
                    "preprocess",
    ...
    def construct_denoiser(self, L, visible):
        ...
        aa_decode_steps = min(
            denoise_kwargs["aa_decode_steps"], denoise_kwargs["partial_T"] or 999
        )
...
class AbSampler(Sampler):
    ...
    def sample_init(self):
        ...
        assert torch.any(self.ab_item.loop_mask), (
    ...
    def sample_step(self, *, t, seq_t, x_t, seq_init, final_step):
        ...
        if (
            ...
            and self.preprocess_conf.msaprev_bugfix
        ):
        ...
        if (t < self.diffuser.T) and (t != self.diffuser_conf.partial_T):
            ...
            xyz_t, t2d, xyz_sc, sc2d = process_selfcond(
                self.prev_pred, t2d, xyz_t, self.ab_conf, xyz_t.device
            )
        ...
        sampled_seq = torch.multinomial(
            seq_probs, 1
        ).squeeze()  # sample a single value from each position

        pseq_0 = (
            torch.nn.functional.one_hot(sampled_seq, num_classes=22)
            .to(self.device)
            .float()
        )

        pseq_0[self.mask_seq.squeeze()] = seq_init[self.mask_seq.squeeze()].to(
            self.device
        )  # [L,22]
