...
class EuclideanDiffuser:
    ...
    def __init__(
        ...
        self.beta_schedule, _, self.alphabar_schedule = get_beta_schedule(
            T, b_0, b_T, schedule_type, **schedule_kwargs
        )
...
class IGSO3:
    ...
    def reverse_sample(
        self, r_t, r_0, t, noise_level, mask=None, rotation_scaling=None
    ):
        ...
        z = torch.Tensor(z.reshape(3)).to(score_approx.device)
        ...
        if rotation_scaling is None:
            perturb_rotvec = (
                rot_g**2
            ) * self.step_size * score_approx + rot_g * np.sqrt(self.step_size) * z
        else:
            perturb_rotvec = (
                (rot_g**2) * self.step_size * score_approx * 0.5 * rotation_scaling
            )

        # Mask perturbation if residue is masked
        if mask is not None:
            perturb_rotvec *= 1 - mask.long()
        ...
        )  # interp_rot represents the sampled r_t-1
        return interp_rot
...
class SLERP:
    ...
    def slerp(self, xyz, diffusion_mask=None):
        ...
        for i in range(len(xyz)):
            ...
            interp_rot = (
                interp_frame.as_matrix().squeeze() @ np.linalg.inv(r_true.squeeze())
            )[None, ...]
        ...
        slerped_crds = (
            np.einsum(
                "lrij,laj->lrai",
                all_interps,
                xyz[:, :3, :] - Ca.squeeze()[:, None, ...].numpy(),
            )
            + Ca.squeeze()[:, None, None, ...].numpy()
        )
...
class INTERP:
    ...
    def interp(self, xyz14, seq, mask14, diffusion_mask, n_steps):
        ...
        if n_steps != 0:
            decode_times, decode_order, idx2steps, aa_masks = get_aa_schedule(
                self.T, L, n_steps
            )
        else:
        ...
        torsions_sincos, torsions_alt_sincos, tors_mask, tors_planar = get_torsions(
            xyz[None], seq[None], TOR_INDICES, TOR_CAN_FLIP, REF_ANGLES
        )

        # convert sin/cos to degrees
        torsions_angle = torch.atan2(
            torsions_sincos[..., 1], torsions_sincos[..., 0]
        ).squeeze() * (180 / np.pi)

        torsions_angle_alt = torch.atan2(
            torsions_alt_sincos[..., 1], torsions_alt_sincos[..., 0]
        ).squeeze() * (180 / np.pi)
        ...
        angle_interpolations = th_interpolate_angles(c, d, self.T, n_diffuse=e).reshape(
            L, 10, self.T
        )
...
class Diffuser:
    ...
    def diffuse_pose(
        ...
        if diffuse_sidechains:
            ...
            assert seq.shape[-1] == L, (
                "Tried to feed non-integer sequence to diffuse torsions"
            )

            # diffuse chi angles/planar angles and sequence information
            tick = time.time()
            diffused_torsions, aa_masks = self.torsion_diffuser.diffuse_torsions(
                xyz[:, :14].clone(),
                seq,
                atom_mask[:, :14].clone(),
            ...
            diffused_torsions_trig = torch.stack(
                [torch.cos(diffused_torsions), torch.sin(diffused_torsions)], dim=-1
            )
            fa_stack = []
            if t_list is None:
                for t, alphas_t in enumerate(diffused_torsions_trig.transpose(0, 1)):
                    ...
                    _, fullatom_t = get_allatom(
                        seq[None], xyz_bb_t[None], alphas_t[None]
                    )
            ...
            if self.aa_decode_steps != 0:
                ...
                _, _, _, aa_masks = get_aa_schedule(
                    self.T, L, 10
                )  # 10 is dummy number. No decoding is happening. TODO clean this up
            ...
            if include_motif_sidechains:
                diffused_fa[:, diffusion_mask, :14, :] = xyz_true[
                    None, diffusion_mask, :14
                ]
