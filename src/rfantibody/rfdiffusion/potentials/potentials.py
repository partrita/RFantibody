...
class olig_contacts(Potential):
    ...
    def __init__(self, contact_matrix, weight_intra=1, weight_inter=1, r_0=8, d_0=2):
        ...
        assert all([i in [-1, 0, 1] for i in contact_matrix.flatten()]), (
...
class olig_intra_contacts(Potential):
    ...
    def compute(self, seq, xyz):
        ...
        """
        assert sum(self.chain_lengths) == len(seq.squeeze), (
            "given chain lengths do not match total sequence length"
        )
...
class glycan_clash(Potential):
    ...
    def __init__(
                ...
                if line[0] == "#":
                    ...
                    if splits[0] == "translation":
                        self.translation = torch.tensor(
                            [float(splits[1]), float(splits[2]), float(splits[3])]
                        )
                    elif splits[0] == "pt1":
        ...
        assert self.translation is not None, (
            "translation not found in point definition file"
        )
    ...
    def compute(self, _, xyz):
        ...
        proj = (torch.sum(v1 * v2, dim=1) / torch.sum(v2 * v2, dim=0))[
            :, None
        ] * v2  # (Lb,3)
